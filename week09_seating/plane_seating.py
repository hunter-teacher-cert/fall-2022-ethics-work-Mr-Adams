...
def main():
 # the biggest ethical concern was for minors to be seperated from their parents who assumed groups would be sat together.  I will prompt and ask parents if they are purchasing a ticket for a minor and have the acknowledge that they may not be able to sit together for the economy price, but the algorithm will make an effort to prioritize them.

    response =str(input("Do you have children under age 13 traveling with you? Y/N  "))
    ans = "y" or "Y"
    if response == ans:
        response =str(input("We will try to seat you and your minor together, but cannot garauntee matching seats. Is this ok with you? Y/N "))
            if response == ans:
                print("your tickets will be processed and seats assigned as best as possible.")  
            else:
